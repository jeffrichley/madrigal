...
from voice.engine.protocol import VoiceInfo


def default_lookup_paths(*, cwd: Path | None = None) -> list[Path]:
    """Return the default tiered lookup paths in priority order.

    Order: local (.voice/voices.yaml under cwd) → project (voices.yaml
    under cwd) → global (~/.config/voice/voices.yaml or
    $XDG_CONFIG_HOME/voice/voices.yaml).
    """
    cwd = (cwd or Path.cwd()).resolve()
    xdg = os.environ.get("XDG_CONFIG_HOME")
    if xdg:
        global_path = Path(xdg) / "voice" / "voices.yaml"
    else:
        global_path = Path.home() / ".config" / "voice" / "voices.yaml"
    return [
        cwd / ".voice" / "voices.yaml",
        cwd / "voices.yaml",
