"""voice.registry — tiered YAML voice catalog.
...
Lookup path order:
    1. ./.voice/voices.yaml         (per-cwd local override)
    2. ./voices.yaml                (project root)
    3. ~/.config/voice/voices.yaml  (user global; honors $XDG_CONFIG_HOME)

First match per ``voice_id`` wins. Whole-voice override only in v0;
per-field partial overlays are a v0.X polish.
"""

from voice.registry.tiered import Registry, default_lookup_paths
