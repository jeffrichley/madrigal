"""voice.cache — content-addressed cache for synthesized audio.

Per plan v2 §2 + §5: minimal 6-field CacheEntry; consumer-wraps-for-extra.
The cache stores ONLY what voice itself cares about (audio bytes + format
...
Key derivation lives in the orchestrator (`voice.generate()`), NOT in
...
Storage: filesystem hash store at ``~/.cache/voice/`` (configurable).
...
from voice.cache.store import Cache, CacheEntry
