...
orchestrator's concern (``voice.generate()`` builds keys from
...
def default_cache_root() -> Path:
    """Default cache root: ~/.cache/voice/ (or $XDG_CACHE_HOME/voice/)."""
    xdg = os.environ.get("XDG_CACHE_HOME")
    if xdg:
        return Path(xdg) / "voice"
    return Path.home() / ".cache" / "voice"
...
class Cache:
    ...
    (``~/.cache/voice/``). All operations are key-addressed; the
